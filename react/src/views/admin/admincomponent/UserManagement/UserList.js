...
function Search() {
  ...
  const initialMembers = [ // 초기 회원 목록 상태 변수  
    { id: 1,image:'대체할 이미지',username:'12445@12321.com',주소:'서초구 서초대로',name: 'John', age: 30, hobby: 'Reading', enroll_date: '2022-01-01' },
    { id: 2, name: '김기기', age: 25, hobby: 'Drawing', enroll_date: '2024-03-04' },
    { id: 3, name: '김기니', age: 40, hobby: 'Music', enroll_date: '2024-03-04' },
    { id: 4, name: '김기가', age: 30, hobby: 'Reading', enroll_date: '2024-03-03' },
    { id: 5, name: '김기나', age: 25, hobby: 'Drawing', enroll_date: '2024-03-03' },
    { id: 6, name: 'Bob', age: 40, hobby: 'Music', enroll_date: '2024-03-02' },
    { id: 7, name: 'John', age: 30, hobby: 'Reading', enroll_date: '2024-03-02' },
    { id: 8, name: 'Alice', age: 25, hobby: 'Drawing', enroll_date: '2024-03-01' },
    { id: 9, name: 'Bob', age: 40, hobby: 'Music', enroll_date: '2024-03-01' },
    { id: 10, name: 'John', age: 30, hobby: 'Reading', enroll_date: '2024-02-29' },
    { id: 11, name: 'Alice', age: 25, hobby: 'Drawing', enroll_date: '2024-02-29' },
  ];
...
// 'handleSearch' 함수 수정
const handleSearch = (selectedDate, nameQuery, titleQuery) => {
  // 이름, 제목, 날짜로 검색된 회원 정보를 필터링합니다.
  const filteredMembers = initialMembers.filter((member) => {
    // 이름, 제목, 날짜에 포함된 검색어가 있는지 확인합니다.
    ...
    const dateMatch = selectedDate && member.enroll_date === selectedDate;

    // 이름, 제목, 날짜 중 하나라도 검색어와 일치하는 경우 필터링합니다.
...
// 이름 입력 필드 변경 처리 함수 수정
const handleNameInputChange = (e) => {
  const value = e.target.value;
  setNameQuery(value);
  if (!value) {
    // 입력값이 비어 있는 경우 모든 회원 데이터를 보여줌
    setMembers(initialMembers);
  } else {
    handleSearch(startDate, value, titleQuery); // 검색 함수 호출
  }
};

// 제목 입력 필드 변경 처리 함수 수정
const handleTitleInputChange = (e) => {
  ...
  if (!value) {
    ...
    handleSearch(startDate, nameQuery, value); // 검색 함수 호출
  }
};
...
const handleKeyPress = (e) => {
  if (e.key === 'Enter') {
    handleSearch(startDate, nameQuery, titleQuery); // 검색 함수 호출
  }
};
// 날짜 선택 핸들러 수정
const handleDateChange = (date) => {
  ...
  handleSearch(formattedDate, nameQuery, titleQuery); // 검색 함수 호출
};
      ...
      <div className="admin-Search">
        ...
        <div className="search-form">
          ...
          <input 
              ...
              onKeyPress={handleKeyPress} // 엔터 키 입력 처리 함수 연결
            />
